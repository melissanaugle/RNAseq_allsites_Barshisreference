...
For all heat versus control modules - blue and dark grey 

```{r setup}
setwd("/Users/Melissa/Desktop/GitHub/RNAseq_allsites_Barshisreference/WGCNA/Allsamples_06_07_2021_simplephenotypes/GO_MWU/")
rm( list = ls())
graphics.off()
library(ggplot2)
library(ggpubr)
...
DGE <- read.csv("../AnnotatedNetworkAnalysisResults1kcontrols_06072021_blue.csv", header = T)
head(DGE)

module_ofinterest <- DGE[DGE$moduleColors == "royalblue",]
names(module_ofinterest)
#change second number based on module 
module_ofinterest <- module_ofinterest[,c(2,60)]
row.names(module_ofinterest) <- NULL
head(module_ofinterest)


#read in file with all contigs from WGCNA file
allcontigs <- as.data.frame(DGE[,1]) #15109 contigs in all modules 
...
contigs_and_signif[!is.na(contigs_and_signif$MM.royalblue),]
#check # or rows to make sure matches expected

contigs_and_signif[is.na(contigs_and_signif$MM.royalblue),2] <- 0
head(contigs_and_signif)
contigs_and_signif[contigs_and_signif$MM.royalblue > 0,]
#check # or rows to make sure matches expected

#write in module name 
write.csv(contigs_and_signif, "contigs_signif_file_GOMWU_royalblue.csv", row.names = F, quote = F)
```
