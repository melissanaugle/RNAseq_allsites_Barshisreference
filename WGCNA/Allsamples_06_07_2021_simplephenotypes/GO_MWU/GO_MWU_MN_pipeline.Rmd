...
module_ofinterest <- DGE[DGE$moduleColors == "darkgrey",]
head(module_ofinterest)
#change second number based on module 
module_ofinterest <- module_ofinterest[,c(2,41)]
...
contigs_and_signif[!is.na(contigs_and_signif$MM.darkgrey),]
#check # or rows to make sure matches expected

contigs_and_signif[is.na(contigs_and_signif$MM.darkgrey),2] <- 0
head(contigs_and_signif)
contigs_and_signif[contigs_and_signif$MM.darkgrey > 0,]
#check # or rows to make sure matches expected

#write in module name 
write.csv(contigs_and_signif, "contigs_signif_file_GOMWU_darkgrey.csv", row.names = F, quote = F)
```
