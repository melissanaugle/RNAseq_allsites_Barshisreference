...
```{r setup}
setwd("/Users/Melissa/Desktop/GitHub/RNAseq_allsites_Barshisreference/WGCNA/Controls_tommodelv2_06_26_2021_tops/GO_MWU/")
rm( list = ls())
graphics.off()
library(ggplot2)
library(ggpubr)
...
DGE <- read.csv("../AnnotatedNetworkAnalysisResults1kcontrols_06232021.csv", header = T)
head(DGE)

lightgreen <- DGE[DGE$moduleColors == "honeydew1",]
head(lightgreen)
#change second number based on module 
lightgreen <- lightgreen[,c(2,45)]
...
contigs_and_signif[!is.na(contigs_and_signif$MM.honeydew1),]
#check # or rows to make sure matches expected

contigs_and_signif[is.na(contigs_and_signif$MM.honeydew1),2] <- 0
head(contigs_and_signif)
contigs_and_signif[contigs_and_signif$MM.honeydew1 > 0,]
#check # or rows to make sure matches expected

#write in module name 
write.csv(contigs_and_signif, "contigs_signif_file_GOMWU_honeydew1.csv", row.names = F, quote = F)
```
