...
```{r setup}
setwd("/Users/Melissa/Desktop/GitHub/RNAseq_allsites_Barshisreference/WGCNA/Controls_tommodelv3_06_29_2021_thesis/GO_MWU/")
rm( list = ls())
graphics.off()
library(ggplot2)
library(ggpubr)
...
DGE <- read.csv("../AnnotatedNetworkAnalysisResults1kcontrols_06292021.csv", header = T)
head(DGE)

module_ofinterest <- DGE[DGE$moduleColors == "darkorange",]
head(module_ofinterest)
#change second number based on module 
module_ofinterest <- module_ofinterest[,c(2,42)]
...
contigs_and_signif[!is.na(contigs_and_signif$MM.darkorange),]
#check # or rows to make sure matches expected

contigs_and_signif[is.na(contigs_and_signif$MM.darkorange),2] <- 0
head(contigs_and_signif)
contigs_and_signif[contigs_and_signif$MM.darkorange > 0,]
#check # or rows to make sure matches expected

#write in module name 
write.csv(contigs_and_signif, "contigs_signif_file_GOMWU_darkorange.csv", row.names = F, quote = F)
```
