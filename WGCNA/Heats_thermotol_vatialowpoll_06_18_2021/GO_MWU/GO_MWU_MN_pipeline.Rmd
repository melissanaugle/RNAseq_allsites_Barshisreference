...
```{r setup}
setwd("/Users/Melissa/Desktop/GitHub/RNAseq_allsites_Barshisreference/WGCNA/Heats_thermotol_vatialowpoll_06_18_2021/GO_MWU/")
rm( list = ls())
graphics.off()
library(ggplot2)
library(ggpubr)
...
DGE <- read.csv("../AnnotatedNetworkAnalysisResults1kcontrols_06182021_lightgreen.csv", header = T)
head(DGE)

lightgreen <- DGE[DGE$moduleColors == "lightgreen",]
lightgreen <- lightgreen[,c(2,49)]
row.names(lightgreen) <- NULL

#read in file with all contigs from WGCNA file
allcontigs <- as.data.frame(DGE[,1]) #11331 contigs in all modules 
...
contigs_and_signif <- merge(lightgreen, allcontigs, all = T)
head(contigs_and_signif)
#12253

contigs_and_signif[!is.na(contigs_and_signif$MM.lightgreen),]
#201 rows, checks out 

contigs_and_signif[is.na(contigs_and_signif$MM.lightgreen),2] <- 0
head(contigs_and_signif)
contigs_and_signif[contigs_and_signif$MM.lightgreen > 0,]
#201 rows, checks out 

write.csv(contigs_and_signif, "contigs_signif_file_GOMWU_lightgreenmodule_WGNCA_heatsonly.csv", row.names = F, quote = F)
```
