...
datExpr = read.csv("../GLM_edgeR_outputs/glmQLF_normalizedCounts.csv")
...
```{r}
datTraits = read.csv("Allsamples_06_07_2021_simplephenotypes/phenotypedata_allsamples_justtreatments_06072021.csv")
head(datTraits)
nrow(datTraits)
...
#INCLUE THE NEXT LINE TO SAVE TO FILE
pdf(file="cluster_allsamples_simplephenotypes.pdf")
...
pdf(file="Trait_heatmap_allsamples_06072021.pdf", height=9, width=8)
labeledHeatmap(Matrix = moduleTraitCor,
xLabels = names(datTraits),
ySymbols = modLabels,
yLabels = modLabels,
...
cex.text = 0.9,
zlim = c(-.8,.8),
main = paste("Module-Trait Correlations at Heat Stress and Control Treatments \n for All Pollution Levels"))
dev.off()
```
...
datOutput=datOutput[order((datOutput$MM.lightsteelblue1),decreasing=T),]
write.table(datOutput,"AnnotatedNetworkAnalysisResults1kcontrols_06072021_lightsteelblue1.csv",row.names=F,sep=",") 
```
...
#Get the gene names
modGenes = names(datExpr)[moduleColors=="lightsteelblue1"]
#Get their uniprot ID's
modUniprotIDs=allUniprotIDs[moduleColors=="lightsteelblue1"];
#Write to a new file and save
fileName = data.frame(modGenes, modUniprotIDs)
write.table(fileName, "lightsteelblue1_Module_genes_uniprot.csv", row.names=F, sep=",")
```
