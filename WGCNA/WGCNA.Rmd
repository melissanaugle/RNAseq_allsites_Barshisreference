...
```{r}
datTraits = read.csv("Controls_tommodelv2_06_26_2021_tops/phenotypedata_controls_tommodels_06_23_21_tops.csv")
head(datTraits)
nrow(datTraits)
...
#could add other gene lists here

```{r}
modulecolor = "antiquewhite1"
...
pdf(file="Trait_heatmap_controls_phenotype_06262021.pdf", height=9, width=8)
labeledHeatmap(Matrix = moduleTraitCor,
xLabels = names(datTraits),
ySymbols = modLabels,
yLabels = modLabels,
...
write.table(datOutput,"AnnotatedNetworkAnalysisResults1kcontrols_06232021.csv",row.names=F,sep=",") 
```
...
#Get the gene names
modGenes = names(datExpr)[moduleColors=="orange"]
#Get their uniprot ID's
modUniprotIDs=allUniprotIDs[moduleColors=="orange"];
#Write to a new file and save
fileName = data.frame(modGenes, modUniprotIDs)
write.table(fileName, "Controls_tommodelv2_06_26_2021_tops/orange_Module_genes_uniprot.csv", row.names=F, sep=",")
```
