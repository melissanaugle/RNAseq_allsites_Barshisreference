...
```{r}
datTraits = read.csv("Heats_tom_models_06_23_2021/phenotypedata_heats_thermotol_tommodel_06232021.csv")
head(datTraits)
nrow(datTraits)
...
Melissa: scale free topology index doesn't pass ~0.82. Is that okay? Continue with 14 for now based on documentation from tutorial: https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/faq.html for soft threshold recs 
...
#INCLUE THE NEXT LINE TO SAVE TO FILE
pdf(file="cluster_heatssamples_vatlow.pdf")
...
And run the following chunk to compare GOterms in the module to GO terms of a type A stress response in Dixon et al 2020:

Don't run this everytime! 
```{r}
module_names <- names(datExpr)[moduleColors=="lightsteelblue1"]
module_names <- as.data.frame(module_names)
annot <- read.csv("../data/33496_MasterCombinedAnnotationTable.csv")
blue_annot <- annot[annot$Ourcontigname%in%module_names$module_names,]
typeAGO <- read.csv("../../../../Desktop/typeA_Dixon_metaanalysis_mec15535-sup-0002-tables1-s10 copy.csv")
head(typeAGO)
blue_annot %>% 
    mutate(GO=strsplit(GO, " // ")) %>% 
    unnest(GO)

GOs_inblue <- blue_annot[blue_annot$GO%in%typeAGO$term,]
#check for dups 
duplicated(GOs_inblue$Ourcontigname)
write_csv(GOs_inblue, "GOs_typeA_dixon_lightsteelblue1.csv")
...
pdf(file="Trait_heatmap_controls_phenotype_rank_06232021.pdf", height=9, width=8)
labeledHeatmap(Matrix = moduleTraitCor,
xLabels = names(datTraits),
ySymbols = modLabels,
yLabels = modLabels,
...
cex.lab.x = 0.8,
zlim = c(-.8,.8),
main = paste("Module-Trait Correlations of Heat Treatments \n for All Pollution Levels"))
dev.off()
```
...
weight = as.data.frame(datTraits$heat_stress); #change to your trait name of interest
names(weight) = "heat_stress"
...
datOutput=datOutput[order((datOutput$MM.lightgreen),decreasing=T),]
write.table(datOutput,"AnnotatedNetworkAnalysisResults1kcontrols_06182021_lightgreen.csv",row.names=F,sep=",") 
```
...
#Get the gene names
modGenes = names(datExpr)[moduleColors=="antiquewhite4"]
#Get their uniprot ID's
modUniprotIDs=allUniprotIDs[moduleColors=="antiquewhite4"];
#Write to a new file and save
fileName = data.frame(modGenes, modUniprotIDs)
write.table(fileName, "Heats_thermotol_06_12_2021/antiquewhite4_Module_genes_uniprot.csv", row.names=F, sep=",")
```
